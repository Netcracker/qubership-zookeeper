...
| global.autoRestartOnSecretChange           | boolean | no        | true          | When `true`, credential Secrets trigger automatic pod restart on rotation. The operator rolls out Deployments when the Secret `resourceVersion` changes. Set `false` for manual rotation. Integration tests use Helm checksum.                                                                                                                                                                         |
...
| pvc.metadata.annotations                   | object  | no        | {}            | The custom annotations added to all Persistent Volume Claims (PVCs) created by the operator.                                                                                                                                                                                                                                                                                                           |
